/**
 * Shared utility for drawing text on canvas (used by both frontend preview and backend)
 * This ensures consistency between preview and final output
 */
export function drawTextOnCanvas(ctx, text, x, y, width, height, options = {}) {
  const {
    fontFamily = 'Arial',
    fontSize = 16,
    color = '#000000',
  ...
  // Set final font and color - EXACT same logic as backend
  const isBold = bold === true || bold === 'bold';
  const fontWeight = isBold ? 'bold' : 'normal';
  ...
  // Try to set font - if it fails, use fallback (same as backend)
  try {
    ctx.font = `${fontWeight} ${finalFontSize}px ${quotedFontFamily}`;
